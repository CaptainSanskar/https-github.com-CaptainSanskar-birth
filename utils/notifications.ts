...
export const BirthdayRepository = {
  ...
  saveBirthdays(birthdays: Birthday[]): void {
    localStorage.setItem('happy4u_birthdays', JSON.stringify(birthdays));
    if (typeof window !== 'undefined' && window.indexedDB) {
      import('./storage').then(m => m.syncToIndexedDB(birthdays));
    }
  }
};
  ...
  getNativeLimit(): number {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (Cap && Cap.Plugins && Cap.Plugins.LocalNotifications) {
        return 500; // Capacitor safely handles hundreds of local elements
      }
    ...
    return 100; // Default browser or lower constraint environment limit
  },
  ...
  async syncWithNativeOS(schedules: ReminderTarget[], birthdays: Birthday[]): Promise<{
    ...
    if (Cap && Cap.Plugins && Cap.Plugins.LocalNotifications) {
      try {
        ...
        const pendingIds = new Set(pendingNotifications.map((n: any) => n.id));
        ...
        pendingNotifications.forEach((n: any) => {
          const birthdayId = n.extra?.birthdayId || (n.data?.birthdayId);
        ...
        const toCancel = pendingNotifications.filter((n: any) => !targetIds.has(n.id));
        ...
        if (toCancel.length > 0) {
          await localNotif.cancel({
            notifications: toCancel.map((n: any) => ({ id: n.id }))
          });
        }
        ...
        if (toSchedule.length > 0) {
          ...
          await localNotif.schedule({ notifications: capNotifs });
        }
